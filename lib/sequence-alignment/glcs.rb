...
class GLCS
  ...
  def initialize(str1, str2, mappings = [])
    ...
    raise ArgumentError, "nil string"     if str1 == nil or str2 == nil
  ...
  def diff
    ...
    begin
      ...
      if s1 != nil
        ...
        diff.push({:action => '+', :old_position => nil, :old_character => nil, :new_position => p2, :new_character => @str2[p2]})
        p2 += 1
      elsif p1 < @len1 && (p2 == @len2 or @glcs[p1][p2 + 1]  < @glcs[p1 + 1][p2])
        diff.push({:action => '-', :old_position => p1, :old_character => @str1[p1], :new_position => nil, :new_character => nil})
        p1 += 1
      end
  ...
  # Returns the side-by-side difference
  def sdiff
    sdiff = []
    ...
    begin
      ...
      if s1 != nil
        ...
        sdiff.push({:action => '=', :old_position => p1, :old_character => s1, :new_position => p2, :new_character => s2})
        p1 += l1; p2 += l2
      elsif p2 < @len2 && (p1 == @len1 or @glcs[p1][p2 + 1] > @glcs[p1 + 1][p2])
        sdiff.push({:action => '+', :old_position => nil, :old_character => nil, :new_position => p2, :new_character => @str2[p2]})
        p2 += 1
      elsif p1 < @len1 && (p2 == @len2 or @glcs[p1][p2 + 1] <= @glcs[p1 + 1][p2])
        sdiff.push({:action => '-', :old_position => p1, :old_character => @str1[p1], :new_position => nil, :new_character => nil})
        p1 += 1
      end
    end until p1 == @len1 && p2 == @len2
  ...
  # Returns the mapping function to convert @str1 to @str2
  # As its side effect, it updates two global variables
  # * common_elements is an array which stores the common elements in the two strings.
  # * mapped_elements is an array which stores the mapped elements in the two strings.
  def mapping
    posmap = {}
    @common_elements = []
    @mapped_elements = []
    ...
    while p1 <= @len1 && p2 <= @len2
      ...
      if s1 != nil
        ...
        posmap[p1] = p2
        (p1 + 1 ... p1 + l1).each{|i| posmap[i] = nil}
        ...
        if !addition.empty? && deletion.empty?
          ...
          if p1 == 0
            ...
            posmap[p1] = p2 - addition.length
          else
            # add mapping for (begin, end)
            posmap[p1] = [p2, p2 - addition.length]
          end
          ...
          deletion[1..-1].each{|p| posmap[p] = nil}
        end
  ...
  # Returns difference of the two strings
  def diff_string
    diff1, diff2 = '', ''
    p1, p2 = 0, 0
  ...
  def _get_glcs_table
    ...
    # compute the GLCS table
    (0...@len1).to_a.reverse.each do |p1|
      (0...@len2).to_a.reverse.each do |p2|
        s1, s2 = _prefix_eq(@str1[p1...@len1], @str2[p2...@len2])
        unless s1 == nil
...
if __FILE__ == $0
  ...
  puts '-----'
  puts "common_elements: #{lcs.common_elements}"
  puts '-----'
  puts "mapped_elements: #{lcs.mapped_elements}"
  puts '-----'
  puts "diff_string: #{lcs.diff_string}"
  puts "front_overflow: #{lcs.front_overflow}"
  puts "rear_overflow : #{lcs.rear_overflow}"
  puts '-----'
  puts "similarity     : #{lcs.similarity}"
  puts "similarity(cut): #{lcs.similarity(true)}"
end
